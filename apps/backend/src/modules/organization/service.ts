...
export const updateOrganization: UpdateOrganizationFn = async (organizationId, data: UpdateOrganizationInput) => {
  ...
  if (hasOneCoordinate(data)) {
    throw httpError("Latitude and longitude must be updated together", 400, "INVALID_ORGANIZATION_LOCATION");
  }
  ...
  if ((nextLatitude === null || nextLongitude === null) && nextRadius !== null) {
    throw httpError("Allowed radius requires latitude and longitude", 400, "INVALID_ORGANIZATION_LOCATION");
  }

  if (nextLatitude !== null && nextLongitude !== null && nextRadius === null) {
    throw httpError("Location requires an allowed radius", 400, "INVALID_ORGANIZATION_RADIUS");
  }
